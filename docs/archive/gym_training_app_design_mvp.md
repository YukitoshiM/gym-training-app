# AIボディメイクマネージャー MVP設計書

作成日: 2026-06-13  
対象: iPhoneアプリ MVP  
元資料: `gym_training_app_user_stories_mvp.md`, `docs/ai_bodymake_manager_product_plan.md`

更新メモ（2026-07-27）:

- 本書はBodyModeの記録基盤を定義した初期MVP設計である。
- Apple Watch、HealthKit、回復分析を含むS01-S58の実装状況は`docs/s01_s58_implementation_backlog.md`を正とする。
- AIコーチ、成果共有、コミュニティ、収益化の拡張設計は`docs/ai_coach_social_monetization_design.md`、S59以降の状況は`docs/s59_growth_implementation_backlog.md`を正とする。

## 1. 概要

本アプリは、ダイエット、筋肥大、健康維持、体型改善、競技力向上をまとめて管理するAIボディメイクマネージャーである。

初期実装では、ジムでの筋力トレーニングを「計画」「実績」「差分」「身体KPI」「次回計画」まで一連のサイクルとして管理する機能を中核にする。その後、体型写真、食事写真、AI週次コメントを接続し、単なる記録アプリではなく「次に何を変えるべきか」を提案する改善管理アプリへ拡張する。
...
7. 食事写真と体型写真を記録し、AI下書きや変化コメントにつなげる
8. オフラインでも記録でき、クラウド同期で記録を失わない
...
| 体型写真 | 正面・横・背面・腹部アップ、写真比較、AI変化コメント | 新規 |
| 食事 | 食事写真、AI下書き、ユーザー補正、PFC合計 | 新規 |
| AI提案 | 週次コメント、停滞原因、次にやること | 新規 |
...
AIボディメイク版のMVPでは下部タブを5つにする。設定はホーム右上またはシートから遷移する。

現在の実装では筋トレ記録を優先しているため「計画」「記録」「履歴」が独立タブになっている。AIボディメイク版へ寄せる段階で、これらは「トレーニング」タブへ統合する。

| タブ | 役割 | 主な画面 |
|---|---|---|
| ホーム | 今日の状態と次にやること | ダッシュボード、記録状況、AIコメント |
| 身体 | 身体KPIと体型写真 | 体重、腹囲、KPIグラフ、写真一覧 |
| 食事 | 食事写真とPFC確認 | 写真登録、AI下書き、ユーザー補正 |
| トレーニング | 計画・記録・履歴 | 計画一覧、開始、記録、履歴、カレンダー、種目別履歴 |
| レポート | 週次まとめとAI提案 | 体重変化、腹囲変化、食事傾向、筋トレ傾向、AIコメント |

設定画面では目的、身長、目標体重、目標腹囲、AI利用設定、ローカルLLM接続設定を扱う。
...
筋トレMEMO、Strong、Hevy、FitNotesなどの公開情報では、種目別履歴、グラフ分析、週次ボリューム、RM計算、前回履歴コピーが主要機能として確認できる。MVPでは、まず入力負荷削減と次回重量判断に直結する分析から実装する。
...
1RM推定はCore後に追加する。RM計算は中上級者には有用だが、初期MVPでは前回履歴表示、前回値コピー、種目別履歴、週次ボリュームを優先する。
...
前回履歴、タイマー、RM、HealthKit、Apple Watch、ウィジェット、回復分析はS01-S58で実装済みである。次は以下の順で拡張する。

1. S59-S66: 日次・週次・月次AIコーチと行動追跡
2. S67-S75: 実績、健康的なストリーク、成果カード、外部共有
3. S76-S81: バーコード、食事提案、定点撮影、写真比較
4. S82-S94: 公開プロフィール、コミュニティ、通報・ブロック
5. S95-S106: チャレンジ、StoreKit、スポンサー、アフィリエイト
6. S107-S114: Garmin、体重計、フォーム動画、法人向け連携

画面、データ境界、プライバシー、Apple審査要件は`docs/ai_coach_social_monetization_design.md`に定義する。

## 19. 未決事項

| 項目 | 選択肢 | 推奨 |
|---|---|---|
| クラウド基盤 | CloudKit / Firebase / Supabase | 個人記録はローカル優先。コミュニティ基盤は運用要件確定後に選定 |
| 年齢管理 | 年齢入力 / 生年月日入力 | プライバシー観点で年齢入力 |
| KPI登録範囲 | プリセット有効化 / 完全カスタム | MVPはプリセット有効化 |
| 重量入力 | 手入力 / 統合リール | 0.1kg単位の統合リールと手入力を併用 |
