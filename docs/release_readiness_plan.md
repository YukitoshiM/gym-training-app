...
| R-012 | 進行中 | Codex/利用者 | Release提出物 | Build 16のアップロードと外部配布は完了。正式公開候補では本番URL、AdMob、Distribution署名Archive、Xcode Privacy Reportを確定する |
| R-013 | 進行中 | Codex/利用者 | ストア素材 | 説明文、キーワード、審査メモ、iPhone 6枚、Watch 3枚は完成。運営者情報と公開URLを入れる |
| R-014 | 進行中 | Codex/利用者 | TestFlight受入 | Build 16を外部TestFlightへ配布済み。iPhoneとWatchで主要フローを一周し、外部テスター5〜10人の受入を完了する |
